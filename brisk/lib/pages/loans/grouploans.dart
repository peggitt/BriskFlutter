...
class _GroupLoansScreenState extends State<GroupLoansScreen> {
  final tabbarlist = [
    translation('grouploans.apply_now'),
    translation('grouploans.active_laons')
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        titleSpacing: 0,
        title: Text(
          getTranslation(context, 'grouploans.loans'),
          style: appBarStyle,
        ),
      ),
      body: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      onChanged: (value) {
                        ...
                        returnMembers[index]['LoanAmount'] = value;
                      },
                    ),
                    heightBox(3.0),
                    TextFormField(
                      ...
                      onChanged: (value) {
                        // Handle changes to the text field
                        returnMembers[index]['Terms'] = value;
                      },
                    ),
                  ],
                ),
    ...
    return GestureDetector(
      onTap: () async {
        ...
        for (int i = 0; i < returnMembers.length; i++) {
          if(returnMembers[i]['LoanAmount']!='') {
            sumLoans = (sumLoans! + double.parse(returnMembers[i]['LoanAmount']));
            if (returnMembers[i]['Terms']=='')
              {
                isComplete = false;
              }
        ...
        if (countLoans == 0)
        {
          showErrorMessage('No Loans Found.');
          return;
        }

        if (isComplete == false)
        {
          showErrorMessage('Confirm Loan term is provided for all loans!.');
          return;
        }
      ...
      child: Container(
        ...
        alignment: Alignment.center,
        child: Text(
          getTranslation(context, 'grouploans.applications'),
          style: bold18White,
        ),
      ),
    );
